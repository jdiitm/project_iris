...
compress_outbound_zstd_test() ->
    ...
    Packet = <<16#11, Payload/binary>>,
    Result = iris_edge_conn:maybe_compress_outbound(Caps, Packet),
    %% B-7 FIX: compress(zstd) returns {error, {zstd_nif_unavailable, _}} when NIF absent.
    %% maybe_compress_outbound uses maybe_compress which gracefully returns {uncompressed, _}.
